...
if title == "Bitcoin: BTC":
    ...
    st.divider()
    st.markdown('''This is an older project that I started as a capstone project for a Python bootcamp. The idea
                for a Blockchain Explorer emerged out of a desire to read the origin block of the Bitcoin blockchain,
                which is the first block created by Satoshi Nakamoto, which has a hidden message for the ages. This was
                also more exciting than the assigned web scraper that was assigned. It works, though it is quite slow
                and the transactions sometimes time out.''')
    st.write( "**Coming Features...**")
